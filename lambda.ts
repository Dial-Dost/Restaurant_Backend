...
// it either way — POST /print/bill and POST /publish/bill are routed to the
// always-on Fargate task by the CloudFront behaviours in deploy/template.yaml.
// ---------------------------------------------------------------------------
...
export const handler: Handler<unknown, unknown> = async (event, context) => {
	...
	// It does NOT confirm Redis received it. The two routes where a lost emit
	// costs money — POST /print/bill (routes/bills.ts, which also emits the
	// per-station KOTs) and POST /publish/bill (routes/bills.ts) — are therefore
	// routed to the always-on task by the CloudFront behaviours in
