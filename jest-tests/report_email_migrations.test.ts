...
// 056-058 land in their own commits (the deploy gate reads them on their own),
// so on the code-only branch the files are absent: every check that reads a
// file skips, and the checks on the runtime's own text still run.
const present = (rel: string): boolean => existsSync(join(__dirname, "..", rel));
const ALL_PRESENT = FILES.every(([file]) => present(file));

describe.each(FILES)("%s", (file, statements) => {
  const text = present(file) ? read(file) : "";
  const body = squash(code(text));

  test("starts with SET LOCAL lock_timeout = '5s', before any statement", () => {
    if (!present(file)) { return; } // shipped in its own commit, applied by hand
...
describe("what the three files say", () => {
  const f056 = ALL_PRESENT ? squash(code(read("migrations/056_report_email_recipients.sql"))) : "";
  const f057 = ALL_PRESENT ? squash(code(read("migrations/057_report_schedule_bundles.sql"))) : "";
  const f058 = ALL_PRESENT ? squash(code(read("migrations/058_report_delivery_bundles.sql"))) : "";

  test("056: the book is tenant data — RLS forced with the house policy — and unique among LIVE addresses", () => {
    if (!ALL_PRESENT) { return; } // shipped in its own commits, applied by hand
  ...
  test("the runtime's probe looks for the LAST thing each file makes", () => {
    ...
    if (!ALL_PRESENT) { return; } // shipped in its own commits, applied by hand
    // ...and each of those really is the last thing its file makes.
