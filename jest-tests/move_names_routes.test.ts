...
describe("POST /tables/move-order — the ticket, by name", () => {
  ...
  test("a manager between two printed bills moves it and is told to reprint BOTH", async () => {
    ...
    // Client items 1-2 at this door: the destination's paper grew, so its
    // addition line is filed AFTER the move line; the source only shrank.
    const reasons = audits().map((a) => a.reason);
    const added = reasons.filter((x) => x.startsWith("ADDED"));
    expect(added).toEqual(["ADDED a move onto the printed bill of table 15 (printed 1 time(s))"]);
    expect(reasons.indexOf(added[0]!)).toBeGreaterThan(reasons.findIndex((x) => x.startsWith("Moved KOT-65")));
  });

  test("a manager onto an unprinted table files no addition line", async () => {
    const r = await moveOrder(MANAGER);
    expect(r.status).toBe(200);
    expect(audits().filter((a) => a.reason.startsWith("ADDED"))).toEqual([]);
  });
