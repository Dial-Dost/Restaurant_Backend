...
describe("an older client's save must not wipe what a newer one added", () => {
  test("the built-ins-only round trip of an old app card KEEPS the stored custom modes", () => {
    const stored = saved([swiggy, { id: "Magicpin", custom: true }]);
    // Exactly what the shipped Flutter card posts: the eight rows it was given,
    // minus anything it does not know how to render.
    const oldCard = stored.filter((m) => !m.custom).map(({ id, label, enabled, requires_screenshot, online }) => ({ id, label, enabled, requires_screenshot, online: online === true }));
