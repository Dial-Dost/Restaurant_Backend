        ...
        try {
            ...
            if (r.ok) {
                ...
                if (!j.mongo || j.mongo.ok === true)
                    return; // proceed when healthy or no mongo field
            }
        }
        catch {
            /* ignore */
...
async function testBestFitAlloc() {
    const now = new Date();
    ...
    if (!postJson.booking_id)
        throw new Error('Missing booking_id in best-fit response');
    if (!postJson.table_name)
        throw new Error('Expected server to allocate a table_name');
    if (postJson.table_name === 'T1')
        throw new Error('Allocated a 2-top for a 3-person party, expected >= 3 capacity');
    // cleanup
...
async function testNoCapacity409() {
    const now = new Date();
...
//# sourceMappingURL=allocation_roundtrip.js.map